...
#define _coro_to_coro_handler(fish) ((coro_handler_t)(fish))

// _coro_to_pool_fish converts a coro_handler_t to pool_fish_t
#define _coro_to_pool_fish(c_hdlr) ((pool_fish_t)(c_hdlr))
...
#define _coro_scheduler_genid(s) ((s)->idgen++)

// _coro_init initializes a coro_t* c, with its corresponding scheduler, id,
// its handler, ex, args, stack, and stacksz
static coro_t *_coro_init(coro_t *c, coro_scheduler_t *s, coro_id_t id,
                          coro_handler_t c_hdlr, coro_ex_t ex, void *args,
...
static void _coro_deinit(coro_t *c) {
  free(c->stack);
  c->stack = NULL;
...
static coro_t *_coro_scheduler_find_coro_by_id(coro_scheduler_t *s,
                                               coro_id_t id) {
  assert(NULL != s);
  ...
  list_foreach_n(&s->delayed_coros, nn) {
    if (list_node_data(nn).c->id == id) {
      c = list_node_data(nn).c;
      return c;
    }
  }
...
static void _coro_scheduler_serve_coro(coro_scheduler_t *s, coro_t *c) {
  assert(NULL != s);
...
static void _coro_scheduler_queue_coro(coro_scheduler_t *s, coro_t *c,
                                       int state) {
...
// _coro_scheduler_suspend_coro suspended c in s, with state
// CORO_STATE_SUSPENDED
static inline void _coro_scheduler_suspend_coro(coro_scheduler_t *s,
                                                coro_t *c) {
...
static inline void _coro_scheduler_prepare_coro(coro_scheduler_t *s,
                                                coro_t *c) {
  _coro_scheduler_queue_coro(s, c, CORO_STATE_READY);
}

// _coro_scheduler_squeeze_coro blews out a coro from pool for a new coro
static coro_handler_t _coro_scheduler_squeeze_coro(coro_scheduler_t *s) {
  assert(NULL != s);
...
static void _coro_scheduler_absorb_coro(coro_scheduler_t *s,
                                        coro_handler_t c_hdlr) {
  assert(NULL != s);
...
static coro_id_t _coro_scheduler_coro(coro_scheduler_t *s, coro_ex_t ex,
                                      void *args) {
  ...
  _coro_init(c, s, _coro_scheduler_genid(s), c_hdlr, ex, args, stack, stacksz);
...
static void _coro_scheduler_yield(coro_scheduler_t *s) {
...
static void _coro_scheduler_delay(coro_scheduler_t *s, long long time) {
...
static void _coro_scheduler_undelay(coro_scheduler_t *s, coro_id_t cid) {
  coro_t *c = NULL;
  int idx = 0;
...
static coro_t *_coro_scheduler_next(coro_scheduler_t *s) {
  ...
  // 2. no delayed coroutines is ready, we pick a suspended one according to
  // FCFS
  //    if we find one in 1(above), it will be the 1st one of suspended coros
  if (0 == list_size(&s->suspended_coros)) {
    return NULL;
  }
...
void coro_scheduler_run(coro_scheduler_t *s) {
  ...
  do {
    ...
    if (NULL == c && _coro_scheduler_has_delayed_coros(s)) {
      continue;
    }
 ...
 */
void yield() { _coro_scheduler_yield(coro_global_scheduler()); }
...
void delay(long long time) {
  if (0 >= time) {
    ...
    _coro_scheduler_delay(coro_global_scheduler(), time);
  }
}
 ...
 * @return id of this coroutine
 */
coro_id_t self() { return coro_global_scheduler()->curr_coro->id; }

/**
 * freeze will make current coroutine delays forever
 */
void freeze() { _coro_scheduler_freeze(coro_global_scheduler()); }
