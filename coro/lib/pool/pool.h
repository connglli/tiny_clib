 ...
 */
#define pool_dead_size(p) ((p)->dead_size)
 ...
 */
#define pool_capacity(p) ((p)->capacity)
 ...
 */
#define pool_is_fish_in_pool(p, fish) (0 <= (fish) && (fish) < (p)->capacity)
...
#define pool_revive(p, fish, x)                                                \
  (pool_is_fish_in_pool((p), (fish)) &&                                        \
       ...
       ? ((p)->fishes[(fish)].state = POOL_FISH_STATE_ALIVE,                   \
          ...
          (p)->alive_tail = (fish), (p)->alive_size++, (fish))                 \
       : (-1))
...
#define pool_zombiate(p, fish)                                                 \
  do {                                                                         \
    if (!pool_is_fish_in_pool((p), (fish))) {                                  \
      break;                                                                   \
    }                                                                          \
    if ((p)->fishes[(fish)].state != POOL_FISH_STATE_ALIVE) {                  \
      break;                                                                   \
    ...
    if (prev == -1) {                                                          \
      ...
      (p)->fishes[prev].next = next;                                           \
    }                                                                          \
    if (next == -1) {                                                          \
      (p)->alive_tail = prev;                                                  \
    } else {                                                                   \
      (p)->fishes[next].prev = prev;                                           \
    }                                                                          \
    (p)->alive_size--;                                                         \
  } while (0)
...
#define pool_kill(p, fish)                                                     \
  do {                                                                         \
    ...
    if ((p)->fishes[(fish)].state == POOL_FISH_STATE_DEAD) {                   \
      break;                                                                   \
    }                                                                          \
    if ((p)->fishes[(fish)].state == POOL_FISH_STATE_ALIVE) {                  \
    ...
    (p)->fishes[(fish)].prev = -1;                                             \
    if ((p)->dead_head != -1) {                                                \
      (p)->fishes[(p)->dead_head].prev = (fish);                               \
    }                                                                          \
    (p)->dead_head = (fish);                                                   \
    (p)->dead_size++;                                                          \
  } while (0)
