...
CORO_EX(fib_writer, args) {
  ...
  if (coro_io_set_nonblock(fd) < 0) {
    fprintf(stderr, "error while fib_writer is setting fd %d to O_NONBLOCK\n",
            fd);
    return NULL;
  }
...
CORO_EX(main_task, args) {
  ...
  for (int idx = 1; idx <= MAX_INDEX; idx++) {
    ...
    if (send(fib_writer_id, MSG_WRITE_ONE, (void *)arg) < 0) {
      ...
      fprintf(stdout, "main_task sent [%d, %lld] to fib_writer %d\n", idx, fib,
              fib_writer_id);
    }
  ...
  if (send(fib_writer_id, MSG_WRITE_OVER, NULL) < 0) {
    ...
    fprintf(stdout, "main_task sent MSG_WRITE_OVER to fib_writer %d\n",
            fib_writer_id);
  }
