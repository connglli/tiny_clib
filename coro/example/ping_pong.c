...
CORO_EX(pong, args) {
  ...
  do {
    ...
    if (send(ping_id, MSG_PING, NULL) != 0) {
      fprintf(stderr, "error while `pong is sending messages MSG_PING to %d`\n",
              ping_id);
      break;
    }
...
CORO_EX(ping, args) {
  ...
  do {
    ...
    if (send(pong_id, MSG_PONG, NULL) != 0) {
      fprintf(stderr, "error while `ping is sending messages MSG_PONG to %d`\n",
              pong_id);
      return NULL;
    }
  ...
  if (send(pong_id, MSG_OVER, NULL) != 0) {
    fprintf(stderr, "error while `ping is sending messages MSG_OVER to %d`\n",
            pong_id);
    return NULL;
  }
