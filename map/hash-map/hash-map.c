 ...
 */
static void *_hash_map_entry_make(int entrysz, const char *k, void *v,
                                  int vsize) {
 ...
 */
static void *_hash_map_find_entry(hash_f_t hash, int *size, void **buckets,
                                  const char *k) {
    ...
    if (strcmp(*_hash_map_entry_unsafe_keyp(curr), k) == 0) {
      if (NULL != prev) {
        *_hash_map_entry_unsafe_bnextp(prev) =
        ...
        buckets[idx] = *_hash_map_entry_unsafe_bnextp(curr);
      }
