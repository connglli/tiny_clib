 ...
 */
#define array_is_index_valid(a, idx) (0 <= (idx) && (idx) < (a)->size ? 1 : 0)
...
#define array_move(a, from_idx, to_idx)                                        \
  (_array_move(array_unstruct(a), array_safe_index((a), (from_idx)),           \
 ...
 */
#define array_get(a, idx) ((a)->data[array_safe_index((a), (idx))])
 ...
 */
#define array_getp(a, idx) (&((a)->data[array_safe_index((a), (idx))]))
  ...
  (((idx) == (a)->size - 1)                                                    \
       ? ((_array_expand(array_unstruct(a)) != 0)                              \
              ...
              : ((a)->data[(idx) + 1] = (a)->data[(idx)],                      \
                 (a)->data[(idx)] = (x), (a)->size++, (idx)))                  \
       : (array_move((a), (idx), (idx) + 1), array_set((a), (idx), (x)),       \
          (idx)))
...
#define array_prepend(a, x)                                                    \
  (((a)->size == 0) ? ((_array_expand(array_unstruct(a)) == 0)                 \
                           ? ((a)->data[(a)->size++] = (x), (0))               \
                           : (-1))                                             \
                    : (array_insert((a), 0, (x))))
...
#define array_swap(a, idx1, idx2)                                              \
  do {                                                                         \
    _array_swap(array_unstruct(a), array_safe_index(a, idx1),                  \
...
#define array_reverse(a)                                                       \
  do {                                                                         \
    _array_reverse(array_unstruct(a));                                         \
  } while (0)
 ...
 * @return   0 if succeeded else -1
 */
#define array_shrink_to_fit(a) (_array_shrink_to_fit(array_unstruct(a)))
 ...
 */
#define array_foreach_i(a, i) for ((i) = 0; (i) < (a)->size; (i)++)
...
#define array_foreach_v(a, v)                                                  \
  for (int _i_d_x = 0; (_i_d_x) < (a)->size && ((v) = (a)->data[_i_d_x], 1);   \
       _i_d_x++)
...
#define array_foreach_p(a, p)                                                  \
  for (int _i_d_x = 0; (_i_d_x) < (a)->size && ((p) = &(a)->data[_i_d_x], 1);  \
       _i_d_x++)
